...
\\
Si possono unire i due nodi $u$ e $v$ in un singolo nodo, ottenendo l'albero $\hat{T}$. Questo è un albero ammissibile per $\{C,f\}$, infatti non scompaiono foglie, e vale
\begin{equation*}
    \forall c \in C : d_{T^*} (c) \geq d_{\hat{T}} (c)
\end{equation*}
Infatti al di fuori del sottoalbero resta tutto inalterato, mentre per le foglie del sottoalbero la profondità scende di 1, e questo succede per \emph{almeno} una foglia $c'$: $d_{\hat{T}} (c') = d_{T^*} (c') -1$
\begin{align*}
    B \left( \hat{T} \right)
    &= \sum_{c \in C} d_{\hat{T}} (c) f(c) \\
    % &= \sum_{c \in C} d_{\widehat{T}} (c) f(c) \\
    &= \sum_{c \in C \setminus \left\{ c' \right\}} d_{\hat{T}} (c) f(c) + d_{\hat{T}} (c') f(c')
\end{align*}
